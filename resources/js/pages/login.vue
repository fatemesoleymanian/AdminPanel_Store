        ...
        <nav class="navbar navbar-expand-lg navbar-light py-3">
            <div class="container">
                ...
                <a href="#" class="navbar-brand">
                    <img src="/images/VdentLogo.png" alt="logo" width="45" height="45" style="border-radius: 50%">
                </a>
            </div>
        </nav>
    </header>
            ...
            <div class="col-md-7 col-lg-6 ml-auto mt-5">
                    <div class="row">
                        ...
                        <div class="input-group mb-3">
                            <span class="input-group-text border-0 bg-light" id="basic-addon2">
                                <i class='bx bxs-envelope text-muted'></i></span>
                            <input type="email" v-model="data.email" class="form-control border-0 bg-light"
                                   placeholder="نشانی ایمیل :" @keyup.enter="login"
                                   aria-label="Recipient's username" aria-describedby="basic-addon2">
                        </div>
                        <!-- Password -->
                        <div class="input-group mb-3">
                            <span class="input-group-text border-0 bg-light" id="basic-addon3">
                                <i class='bx bxs-lock-alt text-muted'></i>
                            </span>
                            <input type="password" class="form-control border-0 bg-light"
                                   v-model="data.password" @keyup.enter="login"
                        ...
                        <div class="input-group mb-3">
                            <span class="input-group-text border-0 bg-light" id="basic-addon4">
                                <i class='bx bxs-lock-alt text-muted'></i>
                            </span>
                            <input type="password" class="form-control border-0 bg-light"
                                   @keyup.enter="login"
                        ...
                        <span class="forget"><router-link to="/forget-password" style="text-decoration: none;
    color: #545b8b;">رمز خود را فراموش کرده اید؟</router-link></span>
                        <div class="row px-3 mb-4">
                            <div class="custom-control custom-checkbox custom-control-inline">
                                <input id="chk1" type="checkbox" name="chk"
                                       v-model="data.rememberMe"
                                       class="custom-control-input">
                                <label for="chk1" class="custom-control-label text-sm text-muted">مرا به خاطر بسپار.</label>
                            </div>
                        </div>
                        <!-- Submit Button -->
                        <div class="row px-3 mb-0 my-2 mx-0">
                            <a href="#" class="btn purple_color btn-block py-2"
                            @click="login">
                                <span class="font-weight-bold">{{ isLoggingIn ? 'در حال پردازش' : 'ورود'}}</span>
                            </a>
                        </div>
                    </div>
            </div>
    ...
    methods:
        {
            async login()
            {
                ...
                if (this.data.email.trim() === '') {
                    ...
                    return this.bottomNotice('لطفا ایمیل را وارد کنید!','error');
                }
                if (this.data.password.trim() === '') {
                    this.isLoggingIn=false
                    return this.bottomNotice('لطفا رمز عبور را وارد کنید!','error');
                }
                if (this.passConfirm.trim() === '') {
                    this.isLoggingIn=false
                    return this.bottomNotice('لطفا نکرار رمز عبور را وارد کنید!','error');
                }
                if (this.passConfirm.trim() !== this.data.password) {
                    this.isLoggingIn=false
                    return this.bottomNotice('لطفا رمز عبور را به درستی وارد کنید!','error');
                }
                const res = await this.callApi('post','app/login_admin',this.data);
                if (res === undefined) this.topNotice('اطلاعات وارد شده صحیح نمیباشد.','error')
                 if (res.status === 200){
                    this.topNotice(res.data.msg,'success');
                     window.location = '/'
                 }
                 else {
                     console.log(res)
                     if(res.status===401){
                         this.topNotice(res.data.msg,'error');
                     }else if(res.status==422){
                         for(let i in res.data.errors){
                             this.topNotice(res.data.errors[i][0])
                         }
                     }
                     else{
                         this.topNotice('مشکلی بوجود آمد لحظاتی دیگر مجددا وارد شوید.','error')
                     }
...
.custom-control-input:focus {
    border-color: #545b8b;
    outline: 0;
    box-shadow: 0 0 0 0.25rem rgba(84,91,139,0.09);
}
#chk1:checked
{
    background-color: #545b8b;
    border-color: #545b8b;
}
.text-color
{
